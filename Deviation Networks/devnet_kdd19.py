...
from utils import dataLoading, aucPerformance, writeResults, get_data_from_svmlight_file,writeperformance
...
def run_devnet(args):
    ...
    for nm in names:
        ...
        for i in np.arange(runs):
            ...
            model = deviation_network(input_shape, network_depth)
            print(model.summary())
            model_name = "sample_data/devnet_" + filename + "_" + str(args.cont_rate) + "cr_" + str(
        ...
        mean_anomalyscore=np.mean(scores)
        std_anomalyscore=np.std(scores)
        upper_threshold=mean_anomalyscore+1.96*std_anomalyscore
        lower_threshold=mean_anomalyscore-1.96*std_anomalyscore
        y_pred=[]
        for itr in range(len(scores)):
             if scores[itr]>upper_threshold or scores[itr]<lower_threshold:
                writeperformance(scores[itr],1,y_test[itr],x_test[itr], path=args.output)
                y_pred.append(1)
             else:
                 writeperformance(scores[itr],0,y_test[itr],x_test[itr], path=args.output)
                 y_pred.append(0)
